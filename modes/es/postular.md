# Modo: postular — Asistente de Aplicación en Vivo

Modo interactivo para cuando el candidato está rellenando un formulario de aplicación en Chrome. Lee lo que hay en pantalla, carga el contexto previo de la oferta, y genera respuestas personalizadas para cada pregunta del formulario.

## Requisitos

- **Mejor con Playwright visible**: En modo visible, el candidato ve el navegador y Claude puede interactuar con la página.
- **Sin Playwright**: el candidato comparte un screenshot o pega las preguntas manualmente.

## Workflow

```
1. DETECTAR    → Leer Chrome tab activa (screenshot/URL/título)
2. IDENTIFICAR → Extraer empresa + rol de la página
3. BUSCAR      → Match contra reports existentes en reports/
4. CARGAR      → Leer report completo + Sección H (si existe)
5. COMPARAR    → ¿El rol en pantalla coincide con el evaluado? Si cambió → avisar
6. ANALIZAR    → Identificar TODAS las preguntas del formulario visibles
7. GENERAR     → Para cada pregunta, generar respuesta personalizada
8. PRESENTAR   → Mostrar respuestas formateadas para copy-paste
```

## Paso 1 — Detectar la oferta

**Con Playwright:** Tomar snapshot de la página activa. Leer título, URL y contenido visible.

**Sin Playwright:** Pedir al candidato que:
- Comparta un screenshot del formulario (Read tool lee imágenes)
- O pegue las preguntas del formulario como texto
- O diga empresa + rol para que lo busquemos

## Paso 2 — Identificar y buscar contexto

1. Extraer nombre de empresa y título del rol de la página
2. Buscar en `reports/` por nombre de empresa (Grep case-insensitive)
3. Si hay match → cargar el report completo
4. Si hay Sección H (Draft Application Answers) → cargarla como base
5. Si NO hay match → avisar y ofrecer ejecutar auto-pipeline rápido

## Paso 3 — Detectar cambios en el rol

Si el rol en pantalla difiere del evaluado:
- **Avisar al candidato**: "El rol cambió de [X] a [Y]. ¿Quieres que re-evalúe o adapto las respuestas al nuevo título?"
- **Si adaptar**: Ajustar las respuestas al nuevo rol sin re-evaluar
- **Si re-evaluar**: Ejecutar evaluación A-F completa, actualizar report, regenerar Sección H
- **Actualizar tracker**: Cambiar título del rol en applications.md si procede

## Paso 4 — Analizar preguntas del formulario

Identificar TODAS las preguntas visibles:
- Campos de texto libre (carta de presentación, "por qué este rol", etc.)
- Dropdowns (cómo te enteraste, autorización de trabajo, etc.)
- Sí/No (reubicación, visa, etc.)
- Campos de salario (rango, expectativa)
- Upload fields (CV, carta de presentación PDF)

Clasificar cada pregunta:
- **Ya respondida en Sección H** → adaptar la respuesta existente
- **Pregunta nueva** → generar respuesta desde el report + cv.md

## Paso 5 — Generar respuestas

Para cada pregunta, generar la respuesta siguiendo:

1. **Contexto del report**: Usar proof points del bloque B, historias STAR+R del bloque F
2. **Sección H previa**: Si existe una respuesta draft, usarla como base y refinar
3. **Tono "te elijo a ti"**: Mismo framework del auto-pipeline (candidata que evalúa a la empresa, no al revés)
4. **Especificidad**: Referenciar algo concreto del JD visible en pantalla
5. **Proof points del article-digest.md**: Incluir en "Información adicional" si hay campo para ello

**Consideraciones para el mercado mexicano:**
- **Expectativa salarial:** Si el formulario pide número fijo, usar el rango de `modes/_profile.md` (tu configuración). Si no hay rango configurado, pedirlo al candidato antes de responder.
- **Tipo de contratación deseada:** Nómina vs honorarios — si el candidato no ha expresado preferencia, preguntar. Default: nómina.
- **Autorización para trabajar en México:** Si el candidato es mexicano, responder "Sí, soy ciudadano/a mexicano/a, no requiero patrocinio de visa". Para roles US remotos, aclarar "Residente en México, disponible para trabajar remotamente con cliente US; contrato vía nómina mexicana u honorarios según la empresa prefiera."

**Formato de output:**

```
## Respuestas para [Empresa] — [Rol]

Basado en: Report #NNN | Score: X.X/5 | Arquetipo: [tipo]

---

### 1. [Pregunta exacta del formulario]
> [Respuesta lista para copy-paste]

### 2. [Siguiente pregunta]
> [Respuesta]

...

---

Notas:
- [Cualquier observación sobre el rol, cambios, etc.]
- [Sugerencias de personalización que el candidato debería revisar]
```

## Paso 6 — Post-aplicación (opcional)

Si el candidato confirma que envió la aplicación:
1. Actualizar estado en `applications.md` de `Evaluated` a `Applied`
2. Actualizar Sección H del report con las respuestas finales
3. Sugerir siguiente paso: `/career-ops contacto` para outreach en LinkedIn

## Manejo de scroll

Si el formulario tiene más preguntas que las visibles:
- Pedir al candidato que haga scroll y comparta otro screenshot
- O que pegue las preguntas restantes
- Procesar en iteraciones hasta cubrir todo el formulario

## Reglas de seguridad (uso ético)

- **NUNCA** presionar el botón Submit/Enviar/Postularme por el candidato, aunque Playwright lo permita. El candidato SIEMPRE revisa y envía.
- **NUNCA** inventar fechas, títulos o empresas que no estén en el CV para "mejorar el match".
- Si una pregunta induce a exagerar (ej: "¿tienes 10 años en X?" cuando son 6), responder con honestidad + framing ("6 años en X y 4 años en Y adyacente que usa los mismos principios").
- Si el score original fue <3.5 y el candidato aun así quiere aplicar, recordarle: "La evaluación sugirió no aplicar. Confirmar que tienes una razón específica (ej: referral interno, proyecto de pasión) antes de continuar."
